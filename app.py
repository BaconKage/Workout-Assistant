...
def posture_check_curl(lm_img, lm_world, debug=None):
    ...
    LSH = lm_img[P.LEFT_SHOULDER.value]
    RSH = lm_img[P.RIGHT_SHOULDER.value]
    LHP = lm_img[P.LEFT_HIP.value]
    RHP = lm_img[P.RIGHT_HIP.value]
    LAK = lm_img[P.LEFT_ANKLE.value]
    RAK = lm_img[P.RIGHT_ANKLE.value]
    ...
    LSHw = lm_world[P.LEFT_SHOULDER.value]
    RSHw = lm_world[P.RIGHT_SHOULDER.value]
    LHPw = lm_world[P.LEFT_HIP.value]
    RHPw = lm_world[P.RIGHT_HIP.value]
    LELw = lm_world[P.LEFT_ELBOW.value]
    RELw = lm_world[P.RIGHT_ELBOW.value]
    ...
    def eval_arm(side):
        if side == "L":
            ...
            vis_ok = visible(lm_img, P.LEFT_SHOULDER.value, P.LEFT_ELBOW.value, P.LEFT_HIP.value)
            pref = "L"
        else:
            SHw, ELw, HPw = RSHw, RELw, RHPw
            vis_ok = visible(lm_img, P.RIGHT_SHOULDER.value, P.RIGHT_ELBOW.value, P.RIGHT_HIP.value)
            pref = "R"
...
def video_frames():
    ...
    while True:
        ...
        # --- Start of Black Screen Placeholder (Standard) ---
        if frame is None:
        ...
        # --- CRITICAL FIX: Add general error handling to the processing loop ---
        try:
            frame_idx += 1
            ...
            if not ok2:
                # If encoding fails, just loop and try again
                time.sleep(0.05)
                continue
            
            # --- Yield the successful frame ---
            yield (b'--frame\r\n'
                   b'Content-Type: image/jpeg\r\n\r\n' + jpeg.tobytes() + b'\r\n')
            
        except Exception as e:
            # --- Error Handling: CRITICAL to keep stream alive ---
            print(f"[MJPEG STREAM ERROR] Unhandled exception in video_frames loop: {e}")
            
            # Yield a broken/black frame and sleep briefly to prevent CPU high usage
            blk = np.zeros((360, 640, 3), dtype=np.uint8)
            cv2.putText(blk, "STREAM ERROR. CHECK LOGS.", (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            ok2, jpeg_blk = cv2.imencode('.jpg', blk, [cv2.IMWRITE_JPEG_QUALITY, 70])
            if ok2:
                 yield (b'--frame\r\nContent-Type: image/jpeg\r\n\r\n' + jpeg_blk.tobytes() + b'\r\n')
...
def analyze():
    ...
    text = "AI analysis failed." # Fallback for OpenAI request
    try:
        ...
        print(f"[OPENAI ERROR] Failed to get response: {e}")
        text = f"OpenAI API failed. Error: {e}"
